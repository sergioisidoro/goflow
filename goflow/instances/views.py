...
def mywork(request, template='goflow/mywork.html'):
    ...
    # workitems = WorkItem.objects.retrieve(user=me, notstatus='complete', noauto=True)
...
def otherswork(request, template='goflow/otherswork.html'):
    ...
    # workitems = WorkItem.objects.retrieve(username=worker, notstatus='complete', noauto=False)
...
def instancehistory(request, template='goflow/instancehistory.html'):
    ...
    inst = ProcessInstance.objects.get(id=id)
...
def activate(request, id):
    ...
    # workitem = WorkItem.objects.get_by(id=id, user=request.user)
